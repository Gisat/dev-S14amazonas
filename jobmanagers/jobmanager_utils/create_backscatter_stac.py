...
##############################################
def group_sarbackscatter_rasters_by_year(input_dir_list, tile_name):
    ...
    for input_dir_list_item in input_dir_list:
        for root, dirs, files in os.walk(input_dir_list_item):
            for filename in files:
                ...
                if str(filename).startswith(f"SARBAC_{tile_name}") and str(filename).endswith("tif"):
                    # Construct the full path to the file
                    file_path = os.path.join(root, filename)
                    # Do something with the .tif file
                    print(f"Processing file: {file_path}")
                    tif_file_date = filename.split("_")[2]
                else:
                    continue
                start_date = datetime.strptime(tif_file_date, "%Y-%m-%d")
                raster_groups[tif_file_date].append(file_path)
...
input_root_dir = Path("/mnt/hddarchive.nfs/amazonas_dir/work_dir/sarbackscatter")
# Name for the base output directory in S3 storage
s3_base_output_dirname = "sarbackscatter"
...
for tile_name_item in tile_name:
    ...
    ##############################################
    ##############################################
    raster_groups = group_sarbackscatter_rasters_by_year([input_root_dir], tile_name_item)
