...
tree_cover_post_composite = Path("/mnt/hddarchive.nfs/amazonas_dir/work_dir/portal_data/post_2021/composites")
os.makedirs(tree_cover_post_composite, exist_ok=True)
...
for tiles_list_item in sorted(tiles_list):
    ...
    portal_data_folder_post_2021_composites_tile = tree_cover_post_composite.joinpath(tiles_list_item)
    ...
    if True:
        ...
        time_windows_3months = get_3months_temporalextents("2021-01-01", "2025-03-31")
        for time_windows_3months_start, time_windows_3months_end in time_windows_3months:
            treecover_change_list = []
            portal_tile_3monthcomposite_tif = portal_data_folder_post_2021_composites_tile.joinpath(f"DEC_3monthcomposite_{tiles_list_item}_{time_windows_3months_start.strftime('%Y-%m-%d')}_{time_windows_3months_end.strftime('%Y-%m-%d')}_TREECOVERCHANGE.tif")
            ...
            if not portal_tile_3monthcomposite_tif.exists():
                ...
                if len(treecover_change_list) == 0: continue
