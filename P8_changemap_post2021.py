...
tree_cover_folder = Path("/mnt/hddarchive.nfs/amazonas_dir/work_dir/portal_data/post_2021/tiles/")
tiles_list = os.listdir(tree_cover_folder)

post_2021_changemap = Path("/mnt/hddarchive.nfs/amazonas_dir/work_dir/portal_data/post_2021/change_map")
os.makedirs(post_2021_changemap, exist_ok=True)


for tiles_list_item in sorted(tiles_list):

    post_2021_changemap_count_tile = post_2021_changemap.joinpath(f"{tiles_list_item}_changemap_count.json")
    post_2021_changemap_ratio_tile = post_2021_changemap.joinpath(f"{tiles_list_item}_changemap_ratio.json")
    post_2021_changemap_total_count_tile = post_2021_changemap.joinpath(f"{tiles_list_item}_total_count.txt")

    if not post_2021_changemap_count_tile.exists():
        ...
        tree_coverchange_tifs = [tree_coverchange_tif_item for tree_coverchange_tif_item in tree_coverchange_tifs
                                 if tree_coverchange_tif_item.endswith("TREECOVERCHANGE.tif")]

        time_windows_24days = get_24day_temporalextents("2022-01-01", "2025-01-01")
        for time_windows_24days_start, time_windows_24days_end in time_windows_24days:
            ...
            for treecover_change_layer_tifs_list_item in sorted(tree_coverchange_tifs):
                treecover_change_layer_tifs_list_item_time = datetime.strptime(
                    treecover_change_layer_tifs_list_item.split("_")[2], "%Y-%m-%d")
            ...
            print(f"- collecting {time_windows_24days_start} - {treecover_change_list}")
        ...
        # Save to JSON file
        with open(str(post_2021_changemap_count_tile), "w") as f:
        ...
        # Save to JSON file
        with open(str(post_2021_changemap_ratio_tile), "w") as f:
            json.dump(tile_time_ratio_json, f, indent=4)

        print(f"total {total}")
        post_2021_changemap_total_count_tile.write_text(str(total))
